...
RATINGFILE = dict({'movielens': 'ratings.txt', 'book': 'ratings_ctr_full.txt', 'movielens20M': 'movie20M_rating.txt'})
LR = dict({'movielens': 1e-2, 'book': 1e-2, 'movielens20M': 5e-3})
L2 = dict({'movielens': 1e-2, 'book': 1e-1, 'movielens20M': 1e-2})
EMB = dict({'movielens': 10, 'book': 20, 'movielens20M': 20})
BOUND = dict({'movielens': 3.5, 'book': 0.5, 'movielens20M': 3.5})
...
def get_envobjects(ratingfile='movielens', max_step=5000, train_rate=0.95,
                   max_stop_count=30):
    ...
    with tf.Session() as sess:
        ...
        pickle_save({'r_matrix': mf_rating, 'user_num': user_num, 'item_num': item_num, 'rela_num': rela_num},
                    '../data/run_time/%s_env_objects' % ratingfile)



#use train user to get popular item for handling totally cold start problem.
def popular_in_train_user(ratingfile='movielens', topk=300, boundary = 3.0):
    ...
    ave_rating = np.mean(r_matrix[:boundary_user_id], axis=0)
